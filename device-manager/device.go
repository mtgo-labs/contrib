...
// (DeviceModel, SystemVersion, LangPack, etc.) for MTProto initConnection.
//
// A [Profile] bundles the device identity fields. It can be applied to a raw
// Config via the convenience [InitConnectionConfig] helper type, or users can
// copy the individual fields directly.
//
// Profiles can be:
...
// InitConnectionConfig mirrors the device fields of a raw.InitConnectionConfig.
// Use it as an intermediate type when transferring a [Profile] into a raw.Config:
//
...
// ToInitConnection converts the Profile into an [InitConnectionConfig].
//
...
//	cfg.InitConnection.DeviceModel = dev.DeviceModel
//	// ...
func (p Profile) ToInitConnection() InitConnectionConfig {
...
// Apply copies the Profile's device identity into an [InitConnectionConfig].
func (p Profile) Apply(cfg *InitConnectionConfig) {
